...
def index():
    ...
    if nyt_key:
        try:
            ...
            news['NYT: Top Stories'] = {"logo": "https://img.logo.dev/nytimes.com?token="+os.environ.get('LOGODEV_TOKEN',''),
                "articles": [{'title': a['title'], 'link': a['url'], 'img': a['multimedia'][0]['url'] if a.get('multimedia') else None} for a in r['results'][:8]]}
        except: pass

    # 2. HACKER NEWS (TECH INTELLIGENCE) - No Key Required
    try:
        r = requests.get("https://hn.algolia.com/api/v1/search?tags=front_page&hitsPerPage=8", timeout=5).json()
        news['Tech Intelligence'] = {"logo": "https://img.logo.dev/ycombinator.com?token="+os.environ.get('LOGODEV_TOKEN',''),
            "articles": [{'title': a['title'], 'link': a['url'] or f"https://news.ycombinator.com/item?id={a['objectID']}", 'img': None} for a in r['hits']]}
    except: pass

    # 3. GUARDIAN API (WORLD NEWS)
    ...
    if g_key:
        try:
            ...
            news['Guardian: Global'] = {"logo": "https://img.logo.dev/theguardian.com?token="+os.environ.get('LOGODEV_TOKEN',''),
    ...
    try:
        ...
        market = [{"s": "BTC", "p": f"${float(btc['data']['amount']):,.0f}"}, {"s": "GOLD", "p": "$2,458"}, {"s": "S&P 500", "p": "5,522"}]
