...
def index():
    ...
    news = {}
    categories = set(['Intelligence', 'Markets', 'Tech'])
    ...
    if nyt_key:
        try:
            ...
            news['NYT'] = {"cat": "Intelligence", "logo": "https://img.logo.dev/nytimes.com?token="+os.environ.get('LOGODEV_TOKEN',''),
    ...
    for f in feeds:
        try:
            ...
            news[f.name] = {"cat": f.category, "logo": f"https://img.logo.dev/{f.url.split('//')[-1].split('/')[0].replace('www.','')}?token={token}", "articles": articles}
