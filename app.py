...
def index():
    ...
    # 1. NYT (PRESTIGE LAYER)
    nyt_key = os.environ.get('NYT_API_KEY')
    if nyt_key:
        try:
            r = requests.get(f"https://api.nytimes.com/svc/topstories/v2/home.json?api-key={nyt_key}", timeout=3).json()
            news['World Report'] = {"logo": "https://img.logo.dev/nytimes.com?token="+os.environ.get('LOGODEV_TOKEN',''),
                "articles": [{'title': a['title'], 'link': a['url'], 'img': a['multimedia'][0]['url'] if a.get('multimedia') else None} for a in r['results'][:6]]}
        except: pass

    # 2. GUARDIAN (GLOBAL LAYER)
    g_key = os.environ.get('GUARDIAN_API_KEY')
    if g_key:
        try:
            r = requests.get(f"https://content.guardianapis.com/search?api-key={g_key}&show-fields=thumbnail&page-size=8", timeout=3).json()
    ...
    for f in feeds:
        try:
            r = requests.get(f.url, headers={'User-Agent': 'Mozilla/5.0'}, timeout=4)
    ...
    # 4. MARKET TERMINAL (Fixed & Expanded)
    market = []
    try:
...
def auto_add():
    ...
    try:
        ...
        res = requests.post("https://openrouter.ai/api/v1/chat/completions", headers={"Authorization": f"Bearer {key}"}, 
            json={"model": "deepseek/deepseek-chat", "messages": [{"role": "user", "content": prompt}]}, timeout=10).json()
